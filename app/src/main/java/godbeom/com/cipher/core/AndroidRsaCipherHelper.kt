...
import godbeom.com.cipher.DLog
...
object AndroidRsaCipherHelper {
    ...
    internal fun init(applicationContext: Context) {
        if (isSupported) {
            DLog.w("Already initialised - Do not attempt to initialise this twice")
            return
        }
        ...
        result = if (keyStore.containsAlias(alias)) {
            ...
            DLog.v("No keypair for %s, creating a new one", alias)

            if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP_MR1 && initAndroidM(alias)) {
                true
            } else {
                initAndroidL(applicationContext,alias)
            }
        }
    ...
    private fun initAndroidM(alias: String): Boolean {
        try {
            ...
            DLog.i("Random keypair with %s/%s/%s is created." + KeyProperties.KEY_ALGORITHM_RSA,
             ...
             */
            DLog.w("It seems that this device does not support latest algorithm!!")

            return false
        }
    }

    private fun initAndroidL(context: Context ,alias: String): Boolean {
        try {
            with(KeyPairGenerator.getInstance(KeyProperties.KEY_ALGORITHM_RSA, KEY_PROVIDER_NAME), {
            ...
            DLog.i("Random keypair with %s is created, which is old fashioned",
                    KeyProperties.KEY_ALGORITHM_RSA)

            return true
        } catch (e: GeneralSecurityException) {
            DLog.w( "It seems that this device does not support encryption!!")
